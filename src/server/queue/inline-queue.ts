...
export class InlineQueue implements TaskQueue {
  ...
  async enqueue(task: QueuedTask) {
    const id = task.type === 'analyze_collection' || task.type === 'transcribe_asset' ? task.jobId : task.type === 'execute_provider_run' ? `${task.type}:${task.providerRunId}` : `${task.type}:${createHash('sha256').update(task.provider).update('\0').update(task.providerArtifactId).digest('hex')}`;
