...
export class OpenAIFactualityAuditor implements FactualityAuditor {
  ...
  async audit(input: FactualityAuditInput) {
    ...
    const executed = await this.executor.execute({
      ...
      canonicalInput: {storyboardId: input.storyboardId, storyboardRevision: input.storyboardRevision, evidenceHash: input.evidenceHash, narrationHash: input.narrationHash, ...(input.auditScope === 'creator_audio' ? {auditScope: input.auditScope, assetId: input.assetId, transcriptId: input.transcriptId, transcriptProviderRunId: input.transcriptProviderRunId} : {}), ...contract},
