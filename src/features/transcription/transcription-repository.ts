...
import {assetTranscripts, assets as mediaAssets, evidenceItems, processingJobs, providerRuns, transcriptEvidenceSegments} from '../../server/db/schema';
...
export type TranscriptionJob = {id: string; projectId: string; assetId: string; providerRunId: string; status: 'pending'|'processing'|'completed'|'failed'|'provider_ambiguous'; leaseToken: string|null; leaseExpiresAt: Date|null};
...
export interface TranscriptionRepository {
  ...
  claimJob(jobId: string, projectId: string, assetId: string, providerRunId: string, leaseMs: number, allowStaleRecovery: boolean): Promise<{outcome: 'claimed'; leaseToken: string}|{outcome: 'busy'|'completed'|'terminal'|'ambiguous'|'missing'}>;
...
export class PostgresTranscriptionRepository implements TranscriptionRepository {
  ...
  async claimJob(jobId: string, projectId: string, assetId: string, providerRunId: string, leaseMs: number, allowStaleRecovery: boolean) {
    ...
    return {outcome: existing.status === 'completed' ? 'completed' as const : existing.status === 'failed' ? 'terminal' as const : existing.status === 'provider_ambiguous' ? 'ambiguous' as const : 'busy' as const};
  }
...
export class InMemoryTranscriptionRepository implements TranscriptionRepository {
  ...
  async claimJob(jobId: string, projectId: string, assetId: string, providerRunId: string, leaseMs: number, allowStaleRecovery: boolean) {
    ...
    if (job.status === 'completed') return {outcome: 'completed' as const}; if (job.status === 'failed') return {outcome: 'terminal' as const}; if (job.status === 'provider_ambiguous') return {outcome: 'ambiguous' as const};
