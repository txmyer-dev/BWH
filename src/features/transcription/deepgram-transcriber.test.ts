...
describe('DeepgramTranscriber', () => {
  it('sends private bytes with mandatory Nova-3 privacy and formatting options', async () => {
    ...
    await new DeepgramTranscriber(client).transcribe({bytes, mimeType: 'audio/wav'});
  ...
  it('passes lease cancellation through and disables request-level retries', async () => {
    ...
    await new DeepgramTranscriber(client).transcribe({bytes: new Uint8Array([1]), mimeType: 'audio/wav', signal: controller.signal});
  ...
  it('normalizes seconds to integer milliseconds and ordered utterance segments', async () => {
    ...
    const transcript = await new DeepgramTranscriber(client).transcribe({bytes: new Uint8Array([1]), mimeType: 'audio/wav'});
  ...
  it.each([
    ...
    await expect(new DeepgramTranscriber(client).transcribe({bytes: new Uint8Array([1]), mimeType: 'audio/wav'}))
