...
export class InMemoryProviderRunRepository implements ProviderRunRepository {
  ...
  constructor(private readonly now: () => Date = () => new Date(), private readonly isLinked: (runId: string) => boolean = () => false, private readonly retireLinkedConsentWork: (runId: string) => void = () => undefined) {}
  ...
  async reserve(input: ReserveProviderRunInput, budgetMicros: number, requestBudget: number) {
    return this.atomic(() => {
      ...
      if (obsolete) { const uncertain = ['dispatching','ambiguous'].includes(obsolete.status); const consentRetired = obsolete.operation === 'transcribe' && ['reserved','processing'].includes(obsolete.status) && obsolete.requestCount === 0; if (consentRetired) this.retireLinkedConsentWork(obsolete.id); obsolete.status = uncertain ? 'superseded_ambiguous' : obsolete.status === 'completed' ? 'completed' : 'failed'; obsolete.settledCostMicros = uncertain ? obsolete.estimatedCostMicros : obsolete.settledCostMicros ?? 0; obsolete.reservedCostMicros = 0; obsolete.activeResult = false; obsolete.leaseToken = null; obsolete.leaseExpiresAt = null; obsolete.dispatchDeadlineAt = null; obsolete.lastError = consentRetired ? 'PROVIDER_PREPARED_CONSENT_CHANGED' : obsolete.lastError; obsolete.updatedAt = this.now(); }
  ...
  async acknowledgeAndRetry(runId: string, budgetMicros: number, requestBudget: number) { return this.atomic(() => {
    const original = this.runs.get(runId); if (!original) throw new Error('PROVIDER_RUN_NOT_AMBIGUOUS');
    if (original.status === 'superseded_ambiguous') { const child = [...this.runs.values()].find((run) => run.retryOfRunId === original.id && run.activeResult && ['reserved','processing','dispatching','completed'].includes(run.status)); if (child) return clone(child); throw new Error('PROVIDER_RUN_NOT_AMBIGUOUS'); }
    if (original.status !== 'ambiguous') throw new Error('PROVIDER_RUN_NOT_AMBIGUOUS');
...
export const providerRunLeasePredicates = {
  claimable: (now: Date): SQL => or(isNull(providerRuns.leaseExpiresAt), lte(providerRuns.leaseExpiresAt, now))!,
  active: (now: Date): SQL => gt(providerRuns.leaseExpiresAt, now),
  activeDispatch: (now: Date): SQL => and(
    gt(providerRuns.leaseExpiresAt, now),
    gt(providerRuns.dispatchDeadlineAt, now)
  )!
};
...
export class PostgresProviderRunRepository implements ProviderRunRepository {
  ...
  async reserve(input: ReserveProviderRunInput, budgetMicros: number, requestBudget: number) {
    return this.database.transaction(async (transaction) => {
      ...
      if (obsolete) { const uncertain = ['dispatching','ambiguous'].includes(obsolete.status); const reservedRequest = ['reserved','processing'].includes(obsolete.status) ? 1 : 0; const consentRetired = obsolete.operation === 'transcribe' && ['reserved','processing'].includes(obsolete.status) && obsolete.requestCount === 0; if (consentRetired) await transaction.update(processingJobs).set({status: 'retired_consent', leaseToken: null, leaseExpiresAt: null, lastError: 'PROVIDER_PREPARED_CONSENT_CHANGED', updatedAt: this.now()}).where(and(eq(processingJobs.projectId, obsolete.projectId), eq(processingJobs.providerRunId, obsolete.id), eq(processingJobs.jobType, 'transcribe_asset'), inArray(processingJobs.status, ['pending','processing']))); await transaction.update(providerRuns).set({status: uncertain ? 'superseded_ambiguous' : obsolete.status === 'completed' ? 'completed' : 'failed', settledCostMicros: uncertain ? obsolete.estimatedCostMicros : obsolete.settledCostMicros ?? 0, reservedCostMicros: 0, activeResult: false, leaseToken: null, leaseExpiresAt: null, dispatchDeadlineAt: null, lastError: consentRetired ? 'PROVIDER_PREPARED_CONSENT_CHANGED' : obsolete.lastError, updatedAt: this.now()}).where(eq(providerRuns.id, obsolete.id)); await transaction.update(projectProviderBudgets).set({reservedMicros: sql`${projectProviderBudgets.reservedMicros} - ${obsolete.reservedCostMicros}`, settledMicros: sql`${projectProviderBudgets.settledMicros} + ${uncertain ? obsolete.estimatedCostMicros : 0}`, reservedRequests: sql`${projectProviderBudgets.reservedRequests} - ${reservedRequest}`, updatedAt: this.now()}).where(eq(projectProviderBudgets.projectId, obsolete.projectId)); }
  ...
  async acknowledgeAndRetry(runId: string, budgetMicros: number, requestBudget: number) { return this.database.transaction(async (transaction) => {
    const [candidate] = await transaction.select({projectId: providerRuns.projectId}).from(providerRuns).where(eq(providerRuns.id, runId)).limit(1); if (!candidate) throw new Error('PROVIDER_RUN_NOT_AMBIGUOUS');
    await transaction.execute(sql`select pg_advisory_xact_lock(hashtext(${candidate.projectId}), hashtext('processing'))`);
    const [original] = await transaction.select().from(providerRuns).where(eq(providerRuns.id, runId)).for('update'); if (!original) throw new Error('PROVIDER_RUN_NOT_AMBIGUOUS');
    if (original.status === 'superseded_ambiguous') { const [child] = await transaction.select().from(providerRuns).where(and(eq(providerRuns.retryOfRunId, original.id), eq(providerRuns.activeResult, true), inArray(providerRuns.status, ['reserved','processing','dispatching','completed']))).limit(1); if (child) return mapRow(child); throw new Error('PROVIDER_RUN_NOT_AMBIGUOUS'); }
    if (original.status !== 'ambiguous') throw new Error('PROVIDER_RUN_NOT_AMBIGUOUS');
