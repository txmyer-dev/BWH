...
import type {DispatchClaim, ProviderRun, ReserveProviderRunInput} from './types';
...
export interface ProviderRunRepository {
  ...
  complete(claim: DispatchClaim, actualCostMicros: number): Promise<void>;
  completeWithResult<T>(claim: DispatchClaim, actualCostMicros: number, persist: () => Promise<T>): Promise<T>;
...
export class InMemoryProviderRunRepository implements ProviderRunRepository {
  ...
  async reserve(input: ReserveProviderRunInput, budgetMicros: number, requestBudget: number) {
    return this.atomic(() => {
      const reusable = [...this.runs.values()].find((run) => run.projectId === input.projectId && run.provider === input.provider && run.model === input.model && run.operation === input.operation && run.inputFingerprint === input.inputFingerprint && run.status === 'completed' && run.activeResult);
      if (reusable) { reusable.cacheHitCount += 1; return {runId: reusable.id, cacheHit: true}; }
      const active = [...this.runs.values()].find((run) => run.projectId === input.projectId && run.provider === input.provider && run.model === input.model && run.operation === input.operation && run.inputFingerprint === input.inputFingerprint && ['reserved', 'processing', 'dispatching', 'ambiguous'].includes(run.status));
  ...
  async complete(claim: DispatchClaim, actualCostMicros: number) { await this.atomic(() => { const run = this.fenced(claim); const now = this.now(); if (run.status !== 'dispatching' || !run.leaseExpiresAt || run.leaseExpiresAt <= now || !run.dispatchDeadlineAt || run.dispatchDeadlineAt <= now) throw new Error('PROVIDER_RUN_FENCED'); run.status = 'completed'; run.settledCostMicros = Math.min(run.reservedCostMicros, Math.max(0, actualCostMicros)); run.reservedCostMicros = 0; run.leaseToken = null; run.leaseExpiresAt = null; run.updatedAt = now; }); }
  async completeWithResult<T>(claim: DispatchClaim, actualCostMicros: number, persist: () => Promise<T>) { return this.atomic(async () => { const run = this.fenced(claim); const now = this.now(); if (run.status !== 'dispatching' || !run.leaseExpiresAt || run.leaseExpiresAt <= now || !run.dispatchDeadlineAt || run.dispatchDeadlineAt <= now) throw new Error('PROVIDER_RUN_FENCED'); const result = await persist(); run.status = 'completed'; run.settledCostMicros = Math.max(0, actualCostMicros); run.reservedCostMicros = 0; run.leaseToken = null; run.leaseExpiresAt = null; run.updatedAt = this.now(); return result; }); }
  ...
  async acknowledgeAndRetry(runId: string, budgetMicros: number, requestBudget: number) { return this.atomic(() => {
    ...
    const id = randomUUID(); const retry: ProviderRun = {...clone(original), id, status: 'reserved', settledCostMicros: null, reservedCostMicros: original.estimatedCostMicros, requestCount: 0, cacheHitCount: 0, leaseToken: null, leaseExpiresAt: null, dispatchDeadlineAt: null, providerIdempotencyKey: `provider-run-${id}`, retryOfRunId: original.id, activeResult: true, lastError: null, createdAt: now, updatedAt: now}; this.runs.set(id, retry); return clone(retry);
  }); }
}

const mapRow = (row: typeof providerRuns.$inferSelect): ProviderRun => ({...row, operation: row.operation as ProviderRun['operation'], status: row.status as ProviderRun['status']});
...
export class PostgresProviderRunRepository implements ProviderRunRepository {
  ...
  async reserve(input: ReserveProviderRunInput, budgetMicros: number, requestBudget: number) {
    return this.database.transaction(async (transaction) => {
      ...
      const [consent] = await transaction.select({id: projectConsents.id}).from(projectConsents).where(and(eq(projectConsents.id, input.consentId), eq(projectConsents.projectId, input.projectId), sql`${projectConsents.invalidatedAt} IS NULL`)).limit(1);
  ...
  async beginDispatch(claim: DispatchClaim, validateConsent?: () => Promise<void>) { await this.database.transaction(async (transaction) => { await validateConsent?.(); const now = this.now(); const [row] = await transaction.update(providerRuns).set({status: 'dispatching', requestCount: sql`${providerRuns.requestCount} + 1`, updatedAt: now}).where(and(eq(providerRuns.id, claim.runId), eq(providerRuns.consentId, claim.consentId), eq(providerRuns.leaseToken, claim.leaseToken), eq(providerRuns.status, 'processing'), sql`${providerRuns.leaseExpiresAt} > ${now}`, eq(providerRuns.activeResult, true), sql`exists (select 1 from ${projectConsents} where ${projectConsents.id} = ${claim.consentId} and ${projectConsents.invalidatedAt} is null)`)).returning(); if (!row) throw new Error('PROVIDER_RUN_FENCED'); await transaction.update(projectProviderBudgets).set({reservedRequests: sql`${projectProviderBudgets.reservedRequests} - 1`, settledRequests: sql`${projectProviderBudgets.settledRequests} + 1`, updatedAt: now}).where(eq(projectProviderBudgets.projectId, row.projectId)); }); }
  ...
  async complete(claim: DispatchClaim, actualCostMicros: number) { await this.database.transaction(async (transaction) => { const now = this.now(); const [before] = await transaction.select().from(providerRuns).where(and(eq(providerRuns.id, claim.runId), eq(providerRuns.leaseToken, claim.leaseToken), eq(providerRuns.status, 'dispatching'), eq(providerRuns.activeResult, true), sql`${providerRuns.leaseExpiresAt} > ${now}`, sql`${providerRuns.dispatchDeadlineAt} > ${now}`)).for('update'); if (!before) throw new Error('PROVIDER_RUN_FENCED'); const settled = Math.min(before.reservedCostMicros, Math.max(0, actualCostMicros)); const [run] = await transaction.update(providerRuns).set({status: 'completed', settledCostMicros: settled, reservedCostMicros: 0, leaseToken: null, leaseExpiresAt: null, updatedAt: now}).where(eq(providerRuns.id, before.id)).returning(); await transaction.update(projectProviderBudgets).set({reservedMicros: sql`${projectProviderBudgets.reservedMicros} - ${before.reservedCostMicros}`, settledMicros: sql`${projectProviderBudgets.settledMicros} + ${settled}`, updatedAt: now}).where(eq(projectProviderBudgets.projectId, run.projectId)); }); }
  async completeWithResult<T>(claim: DispatchClaim, actualCostMicros: number, persist: () => Promise<T>) { return this.database.transaction(async (transaction) => { const now = this.now(); const [before] = await transaction.select().from(providerRuns).where(and(eq(providerRuns.id, claim.runId), eq(providerRuns.leaseToken, claim.leaseToken), eq(providerRuns.status, 'dispatching'), eq(providerRuns.activeResult, true), sql`${providerRuns.leaseExpiresAt} > ${now}`, sql`${providerRuns.dispatchDeadlineAt} > ${now}`)).for('update'); if (!before) throw new Error('PROVIDER_RUN_FENCED'); const result = await persist(); const settled = Math.min(before.reservedCostMicros, Math.max(0, actualCostMicros)); await transaction.update(providerRuns).set({status: 'completed', settledCostMicros: settled, reservedCostMicros: 0, leaseToken: null, leaseExpiresAt: null, updatedAt: this.now()}).where(eq(providerRuns.id, before.id)); await transaction.update(projectProviderBudgets).set({reservedMicros: sql`${projectProviderBudgets.reservedMicros} - ${before.reservedCostMicros}`, settledMicros: sql`${projectProviderBudgets.settledMicros} + ${settled}`, updatedAt: this.now()}).where(eq(projectProviderBudgets.projectId, before.projectId)); return result; }); }
  ...
  async acknowledgeAndRetry(runId: string, budgetMicros: number, requestBudget: number) { return this.database.transaction(async (transaction) => {
    ...
    await transaction.execute(sql`select pg_advisory_xact_lock(hashtext(${original.projectId}))`);
    const [consent] = await transaction.select({id: projectConsents.id}).from(projectConsents).where(and(eq(projectConsents.id, original.consentId), sql`${projectConsents.invalidatedAt} IS NULL`)).limit(1); if (!consent) throw new Error('PROCESSING_CONSENT_REQUIRED');
