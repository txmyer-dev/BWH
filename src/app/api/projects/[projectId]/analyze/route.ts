...
export async function POST(_request: Request, context: RouteContext) {
  ...
  try {
    ...
    await new ConsentService(new PostgresConsentRepository(database), async () => undefined).assertProcessingConsent(projectId, 'google_gemini', ['selected_photos', 'captions', 'written_artifacts', 'transcripts']);
    ...
    const service = new EvidenceService(
      ...
      storyAgent,
      new PrivateAnalysisAssetSource(new PostgresAssetRepository(database), new GcsMediaStorage(env.GCS_BUCKET)),
