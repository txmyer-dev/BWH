...
export async function POST(_request: Request, context: RouteContext) {
  ...
  try {
    ...
    const auditDeployment = factualityAuditDeployment(env);
    ...
    await new ConsentService(new PostgresConsentRepository(database), async () => undefined).assertProcessingConsent(projectId, auditDeployment.provider, ['approved_narration_text', 'source_references']);
    ...
    if (!auditor) throw new Error('FACTUALITY_AUDIT_NOT_CONFIGURED');
